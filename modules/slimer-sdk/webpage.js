...
function _create(parentWebpageInfo) {
    ...
    function getNetLoggerOptions(webpage, deferred, firstRequestHeaders) {
        ...
        return {
            ...
            onLoadFinished: function(url, success, statusCode){
                if (wycywigReg.test(url)) {
                    return;
                }
                webpage.loadFinished(success, url, false, statusCode);
    ...
    var webpage = {
        ...
        loadFinished: function(status, url, isFrame, statusCode) {
            browserJustCreated = false;
            webPageSandbox = null;
            if (DEBUG_WEBPAGE_LOADING) {
                slDebugLog("webpage: onLoadFinished status:"+status+" url:"+url+" isFrame:"+isFrame);
            }
            executePageListener(this, 'onLoadFinished', [status, url, isFrame, statusCode]);
        },
