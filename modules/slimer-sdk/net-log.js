...
let recordRequestStackTraces = false;

exports.enableRequestStackTraceRecording = function (val) {
    if (typeof val !== 'boolean') {
        throw new Error(
            "Value for recordRequestStackTraces should be boolean, got" + val
        );
    }
    recordRequestStackTraces = val;
}

function getRequestStack(stack) {
    // skip frames belonging to our chrome code
    stack = stack.caller.caller.caller.caller;

    if (stack === null) {
        return null;
    }

    const formattedStack = stack.formattedStack.split('\n');
    const stacktrace = [];

    let i = 0;

    while(stack !== null) {
        let uri = formattedStack[i].split('@')[1];
        ...
        stacktrace.push({
            name: stack.name,
            lineNumber: stack.lineNumber - 1,
            columnNumber: stack.columnNumber - 1,
            sourceLine: stack.sourceLine,
            pretty: formattedStack[i],
            fileURI: uri,
            // asyncCause object null
            // asyncCaller object null
            // caller object
...
const traceRequest = function(id, request) {
    ...
    const loadType = request.loadInfo.externalContentPolicyType;
    ...
    let res = {
        ...
        time: new Date(),
        headers: headers,
        isXHR: loadType === Ci.nsIContentPolicy.TYPE_XMLHTTPREQUEST,
        isFetch: loadType === Ci.nsIContentPolicy.TYPE_FETCH,
        loadType: loadType,
        stacktrace: null,
    };

    if (recordRequestStackTraces && loadType !== Ci.nsIContentPolicy.TYPE_DOCUMENT) {
        res.stacktrace = getRequestStack(components.stack);
    }
            ...
            if (this.isLoaded(flags)) {
                ...
                if (typeof(this.options.onLoadFinished) === "function") {
                    ...
                    Services.tm.currentThread.dispatch({ run: function(){
                        opt.onLoadFinished(uri, success, responseStatus);
