...
function outputStr(str, isError) {
    if ((slConfiguration.isWindows || slConfiguration.outputEncoding =='UTF-8'
        || slConfiguration.outputEncoding =='binary') && !isError) {
        dump(str);
        return;
    }
    try {
        if (outputStream) {
            outputStream(str, isError);
            return;
        }
        var outStream = openFileStream('/dev/stdout');
        var errStream = openFileStream('/dev/stderr');
        var uconv = Cc["@mozilla.org/intl/scriptableunicodeconverter"]
            .createInstance(Ci.nsIScriptableUnicodeConverter);
        uconv.charset =  slConfiguration.outputEncoding;
        outputStream =  function (str, isError) {
            let istream = uconv.convertToInputStream(str);
            let len = istream.available();
            while (len > 0) {
                if (isError) {
                    errStream.writeFrom(istream, len);
                } else {
                    outStream.writeFrom(istream, len);
                }
...
function dumpArguments() {
    for (var i = 0; i < arguments.length; i++) {
        outputStr(arguments[i] + " ", this.isError);
    }
    outputStr('\n', this.isError);
}
...
const dumpArgumentsNotErr = dumpArguments.bind(isNotErr);
const dumpArgumentsErr = dumpArguments.bind(isErr);
slConsole.prototype = {
    log:dumpArgumentsNotErr,
    info:dumpArgumentsNotErr,
    warn:dumpArgumentsNotErr,
    error:dumpArgumentsErr,
    debug:dumpArgumentsNotErr,
